'''
Biddle et al 2020 manuscript Figure 7:
Caption:
Model prediction summary for FLT during Lee with vegetation (solid) and without vegetation (dashed).
From top to bottom the parameters are; Q - River discharge (m3/s), SSCf - Depth averaged fine sediment concentration
(kg/m3), SSCc - Depth averaged coarse sediment concentration (kg/m3), hb - Bed thickness (m), |v ̅ | - Depth averaged
current magnitude (m/s), |τ_b | - Maximum wave and current bottom stress magnitude (N/m2) with a dotted line
indicating the critical shear stress of 0.049 N/m2.
...
vars2plot = ['river_transport','mud_bar','sand_bar','bed_thickness','current_bar','bstress_mag']
#i=0
...
labels = ['$Q$ ($m^3$ $s^{-1}$)','$\\widebar{SSC}_{f}$ (kg $m^{-3}$)','$\\widebar{SSC}_{c}$ (kg $m^{-3}$)','$h_b$ ($m$)',
...
import datetime
# set xlim
ax[5].set_xlim(point_data_veg[site][vars2plot][start:end].index.min(),
